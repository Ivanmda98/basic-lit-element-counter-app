...
export class CounterComponent extends LitElement {
  ...
  _onClickDecrement() {
    if (this._count === 0) {
      this.dispatchEvent(
        new CustomEvent(`${CounterComponent.is}-count-zero`, {
          bubbles: true,
          composed: true,
        }),
      );
    ...
    return html`
      <div class="counter-component-container">
        ...
        <div class="counter-buttons-container">
          <button id="increment-button" @click=${this._onClickIncrement}>
            Increment
          </button>
          <button id="decrement-button" @click=${this._onClickDecrement}>
            Decrement
          </button>
          <p>${this.algo}</p>
        </div>
